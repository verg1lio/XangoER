...
class Motor:
    ...
    def lista_torque(self):
        lista_torque_mec = np.array (self.torque_mecanico [:167])
        return lista_torque_mec
    ...
    def example():
        ...
        return motor  # Retorna a instância do motor

    @staticmethod
    def valor_de_torque(motor_instance):
        motor_instance.simulate()
        val_torque = np.array(motor_instance.torque_mecanico)
        df = pd.DataFrame(val_torque[:167], columns=['Torque (N*m)'])
        df.to_excel('torque_mecanico.xlsx', index=False)
...
# exemplos
motor_instance = Motor.example()
Peso.example()
torque_values = Motor.valor_de_torque(motor_instance)
