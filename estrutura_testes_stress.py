...
nodes_otimizada = pd.read_csv('solucao_otimizada_nodes.csv')   # coloque o nome correto do arquivo

# 2. Remover a coluna de índice (primeira coluna)
nodes_otimizada = nodes_otimizada.drop(columns=['index'])   # ou o nome real da primeira coluna

# 3. Converter para numpy array
nodes = nodes_otimizada.to_numpy()

elements_otimizada = pd.read_csv('solucao_otimizada_elements.csv')

# Converter cada linha em uma tupla
elements = list(elements_otimizada.itertuples(index=False, name=None))
...
#F_global[1*6+2] = 100
fixed_dofs = []
fixed_nodes = [24, 25, 26, 27]

for j in fixed_nodes:
    for i in range(0, 6):
        fixed_dofs.append(j*6 + i)

# Perform deformation analysis

pack_sup1,pack_sup2 = [14, 15]
pack_sup3,pack_sup4 = [20, 21]

for i in [pack_sup1,pack_sup2,pack_sup3,pack_sup4]:
    F_global[i * 6 + 2] = (65 * 9.81) / 4
