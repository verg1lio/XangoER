...
# ---------------------
    ...
    def mass(self):
        ...
        for element in self.elements:
            Le = self.calcular_comprimento(element)
    ...
    def calcular_K_Elementar(self,element):
        ...
        Ke *= Le  # fator Jacobiano (dy = L dxi)
        node1, node2, tube_type = element
        # Coordenadas dos nós
...
# ---------------------
    ...
    def element(self, element):
        ...
        c1 = (E * A) / Le
        c2 = (G * J) / Le
        c3 = (E * I) / Le**3               #Euler-Bernoulli
        c4 = (E*I)/(Le**3*(Phi+1))         #Timoshenko
        t1 = (4+Phi)
        t2 = (2-Phi)
        d1 = rho*A*Le
        d2 = (I*Le)/6
        d3 = (rho*A*Le)/420
        c5 = (4*E*I*((kappa*G*A)**2*Le**4+3*kappa*G*A*Le**2*E*I+36*(E*I)**2))/(Le*(12*E*I - kappa*G*A*Le**2)**2)
        c6 = -(2*E*I*(72*(E*I)**2-(kappa*G*A)**2*Le**4 - 6*kappa*G*A*Le**2*E*I))/(Le*(12*E*I - kappa*G*A*Le**2)**2)

        k_e = np.array([
            [12*c4, 0, 0, 0, 0, 6*Le*c4, -12*c4, 0, 0, 0, 0, 6*Le*c4],
            [0, c1, 0, 0, 0, 0, 0, -c1, 0, 0, 0, 0],
            [0, 0, 12*c4, 6*Le*c4, 0, 0, 0, 0, -12*c4, 6*Le*c4, 0, 0],
            [0, 0, 6*Le*c4, c5, 0, 0, 0, 0, -6*Le*c4, c6, 0, 0],
            [0, 0, 0, 0, c2, 0, 0, 0, 0, 0, -c2, 0],
            [6*Le*c4, 0, 0, 0, 0, c5, -6*Le*c4, 0, 0, 0, 0, c6],
            [-12*c4, 0, 0, 0, 0, -6*Le*c4, 12*c4, 0, 0, 0, 0, -6*Le*c4],
            [0, -c1, 0, 0, 0, 0, 0, c1, 0, 0, 0, 0],
            [0, 0, -12*c4, -6*Le*c4, 0, 0, 0, 0, 12*c4, -6*Le*c4, 0, 0],
            [0, 0, 6*Le*c4, c6, 0, 0, 0, 0, -6*Le*c4, c5, 0, 0],
            [0, 0, 0, 0, -c2, 0, 0, 0, 0, 0, c2, 0],
            [6*Le*c4, 0, 0, 0, 0, c6, -6*Le*c4, 0, 0, 0, 0, c5]
        ])
        
        # Matriz de Massa Elementar
        m_e= np.array([
                [156 * d3, 0, 0, 22 * Le * d3, 0, 0, 54 * d3, 0, 0, -13 * Le * d3, 0, 0],
                [0,2*d1, 0, 0, 0, 0, 0, d1, 0, 0, 0, 0],
                [0, 0, 156 * d3, 0, 0, 22 * Le* d3, 0, 0, 54 * d3, 0, 0, -13 * Le * d3],
                [22 * Le * d3, 0, 0, 4 * Le**2 * d3, 0, 0, 13 * Le * d3, 0, 0, -3 * Le**2 * d3, 0, 0],
                [0, 0, 0, 0, 2*d2, 0, 0, 0, 0, 0, d2, 0],
                [0, 0, 22 * Le * d3, 0, 0, 4 * Le**2 * d3, 0, 0, 13 * Le * d3, 0, 0, -3 * Le**2 * d3],
                [54 * d3, 0, 0, 13 * Le * d3, 0, 0, 156* d3, 0, 0, -22 * Le * d3, 0, 0],
                [0, d1, 0, 0, 0, 0, 0, 2*d1, 0, 0, 0, 0],
                [0, 0, 54 * d3, 0, 0, 13 * Le * d3, 0, 0, 156 * d3, 0, 0, -22 * Le * d3],
                [-13 * Le * d3, 0, 0, -3 * Le**2 * d3, 0, 0, -22 * Le * d3, 0, 0, 4 * Le**2 * d3, 0, 0],
                [0, 0, 0, 0, d2, 0, 0, 0, 0, 0, 2*d2, 0],
                [0, 0, -13 * Le * d3, 0, 0,-3 * Le**2 * d3, 0, 0, -22 * Le * d3, 0, 0, 4 * Le**2 * d3]
            ])
        return k_e,m_e
    ...
    def shape_fun(self, F_flexao1=1000, F_flexao2=1000, F_axial=1000,F_torcao=1000): 
        ...
        for element in self.elements:
            ...
            Le = self.calcular_comprimento(element)
            # Equação de torsão
            torcao_val = (F_torcao * Le) / (G * J)         #Fonte[1]
            torcao.append(torcao_val)
            # Equação  para deformação axial
            deformacao_val = (F_axial* Le / (A * E))       #Fonte[2]
            deformacao.append(deformacao_val)
            # Equação para flexão
            flexao_val1 = (F_flexao1*Le**3)/(48 * E * I)      #Fonte[3.1] (carga pontual no meio do elemento biapoiado)
            flexao_val2 = (5*F_flexao2*Le**4)/(384 * E * I)   #Fonte[3.2] (carga distribuída ao longo de todo o elemento biapoiado)
    ...
    def calcular_B_Elementar(self, element,xi=0.5):
        ...
        I, J = self.momento_inercia_area_e_polar(dimension, e, shape)
        k = 0.9  
        alfa = 12.0 * E * I / (k * G * A * Le**2)
        beta = 1.0 / (1.0 + alfa)        
        # Funções consistentes (uma vez só!)
        p = self.funcoes_de_forma(xi, Le,alfa,beta)
        invL = 1.0 / Le
...
# ---------------------
    ...
    def calcular_centro_massa(self):
        """Método oficial da classe para calcular centro de massa"""
        total_mass = 0.0
        ...
        for element in self.elements:
            ...
            length = self.calcular_comprimento(element)
            centroid = 0.5 * (p1 + p2)
            
            # Usa os métodos internos da classe
            E, G, dimension, e, rho, poisson, shape = self.obter_propriedades(tube_type)
            A = self.area_seccao_transversal(dimension, e, shape)
        ...
        center_of_mass = weighted_sum / total_mass if total_mass > 0 else np.zeros(3)
        return center_of_mass, total_mass
    
    def plotar_centro_massa(self):
        """Plota a estrutura com o centro de massa destacado"""
        cm, massa_total = self.calcular_centro_massa()
        ...
        # Plotar centro de massa
        ax.scatter(cm[0], cm[1], cm[2], 
                  color='red', s=300, marker='*',
                  label=f'Centro de Massa\n({cm[0]:.3f}, {cm[1]:.3f}, {cm[2]:.3f})\nMassa: {massa_total:.1f} kg')
        
        # Ajustar proporção dos eixos
        x_coords = [p[0] for p in self.nodes]
        y_coords = [p[1] for p in self.nodes]
        z_coords = [p[2] for p in self.nodes]
        ...
        ax.set_ylabel('Y (m)')
        ax.set_zlabel('Z (m)')
        ax.set_title('Estrutura com Centro de Massa')
        ax.legend(loc='upper left')
        plt.tight_layout()
        plt.show()
        
        print(f"Centro de massa: ({cm[0]:.3f}, {cm[1]:.3f}, {cm[2]:.3f}) m")
        print(f"Massa total: {massa_total:.2f} kg")
...
print(K_global) 
x = estrutura.calcular_K_Elementar(elements[0])
print(x)
...
print("=" * 60)
print("INICIANDO ANÁLISE COMPLETA DA ESTRUTURA")
print("=" * 60)
...
print(f"✓ Estrutura criada com {estrutura.num_elements} elementos e {estrutura.num_nodes} nós")
print(f"✓ Propriedades de {len(estrutura.tube_properties_cache)} tipos de tubo carregadas")

# 3. Cálculos básicos
massa_total = estrutura.mass()
print(f"✓ Massa total da estrutura: {massa_total:.2f} kg")

comprimentos = estrutura.calcular_comprimentos_e_quantidades_por_tipo()
print("✓ Comprimentos por tipo de tubo:")
for tipo, dados in comprimentos.items():
    print(f"  - {tipo}: {dados['quantidade']} unidades, {dados['comprimento_total']:.3f} m total")

# 4. Construir matrizes globais
print("✓ Construindo matrizes globais...")
K_global, M_global = estrutura.matrizes_global()

# 5. Aplicar condições de contorno (engastar primeiro nó)
print("✓ Aplicando condições de contorno...")
estrutura.aplicar_engastes([0], [0, 1, 2, 3, 4, 5])  # Engasta todos os DOFs do nó 0

# 6. Visualizações
print("✓ Gerando visualizações...")

# 6.1 Plotagem 3D básica
print("  - Plotagem 3D do chassi")
estrutura.plotar_chassi_3d()

# 6.2 Análise de formas modais
print("  - Análise modal")
estrutura.modal_analysis_plot()

# 6.3 Análise de funções de forma
print("  - Análise de deformações")
estrutura.shape_fun_plot(F_flexao1, F_flexao2, F_axial, F_torcao)

# 7. Cálculos adicionais para demonstração
print("✓ Realizando cálculos adicionais...")

# Matriz de rigidez elementar
ke_elementar = estrutura.calcular_K_Elementar(elements[0])
print(f"✓ Matriz de rigidez do primeiro elemento calculada ({ke_elementar.shape})")

# Análise modal numérica
autovalores, autovetores, frequencias = estrutura.modal_analysis()
print(f"✓ Análise modal concluída - {len(frequencias)} modos calculados")
