...
class Motor:
    ...
    def load_torque(self,):
        """Calcula torque resistente inercial: tau_load = J_eff * alpha."""
        alpha = (self.wm - self.prev_wm) / self.h  # aceleração angular (rad/s²)
        self.cl = self.J_eff * alpha      
...
class Drivetrain:
    ...
    def Reduções(self, eficiencia_corrente=0.96):
        ...
        # Calcula inércia efetiva (rodas + transmissão) refletida ao motor
        R_total = self.reducao_primaria * self.reducao_final
        r = self.raio_pneu * 0.001  # [m]
        J_wheel = 0.5 * self.massa_roda * r**2      # uma roda (cilíndro sólido)
        J_wheel_total = 2 * J_wheel                 # duas rodas traseiras motrizes
        motor.J_eff = J_wheel_total * (R_total**2)
    ...
    def CarPerformance(self):
        ...
        J_wheel = 0.5 * self.massa_roda * r**2   # inércia de uma roda
        J_wheel_total = 2 * J_wheel              # duas rodas motrizes
        J_car_equiv = self.massa * r**2          # inércia equivalente do carro no raio da roda
        velocidade_angular = 0.0                # velocidade angular inicial (rad/s)
        ...
        for _, torque, tempo in zip(rpm_reduzido, torque_reduzido, variacao_tempo):
            ...
            # Torque efetivo após perdas de transmissão
            Torque_eff = torque * rendimento_transmissao

            # Distribui torque entre girar rodas e acelerar o carro
            J_total = J_wheel_total + J_car_equiv
           
            torque_to_wheel = Torque_eff * (J_wheel_total / J_total)
            torque_to_car = Torque_eff * (J_car_equiv  / J_total)
           
            # Atualiza velocidade angular da roda
            
            aceleracao_angular = torque_to_wheel / J_wheel_total
            velocidade_angular += aceleracao_angular * dt

            # Força trativa gerada (torque aos pneus sobre o carro)
            forca_trativa = torque_to_car / r

            # Resistências (rolamento e arrasto)
            resistencia_rolamento = c_r * peso
            forca_arrasto = (densidade_ar * velocidade_linear_ms**2 * coeficiente_arrasto * area_frontal) / 2

            # Força líquida no veículo
            forca_final = max(forca_trativa - forca_arrasto - resistencia_rolamento, 0.0)

            # Atualiza velocidade linear do carro (F = m a)
            aceleracao = forca_final / self.massa
            velocidade_linear_ms += aceleracao * dt
