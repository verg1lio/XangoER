...
def simulate_car(F_pedal, pos_acel_max, pos_freio, pos_desejada):
    ...
    a_acel = (V1**2) / (2 * pos_acel_max)  # Aceleração constante até pos_acel_max
    ...
    return abs(pos_final - pos_desejada), pos_acel_max, pos_freio, pos_final, t_values, acceleration, velocity, position
...
def differential_evolution(pos_desejada, F_min=445, F_max=823, pos_min=0, pos_max=1000, pop_size=4000, max_iter=6400, CR=0.85, F_scale=0.5, tol=10e-5):
    ...
    for iteration in range(max_iter):
        for i in range(pop_size):
            ...
            # Seleção
            fitness_trial, pos_acel_trial, pos_freio_trial, pos_final_trial, _, _, _, _ = simulate_car(trial_F, trial_pos_acel, trial_pos_freio, pos_desejada)
            fitness_current, pos_acel_current, pos_freio_current, pos_final_current, _, _, _, _ = simulate_car(population_F[i], population_pos_acel[i], population_pos_freio[i], pos_desejada)
...
def generate_animation_and_plots(F_pedal, pos_acel_max, pos_freio, pos_desejada):
    ...
    m = 230  # Massa do carro (kg)
    amax = F_pedal / m  # Desaceleração máxima (m/s²)
    V2 = 26.7 - (0.5 * amax * tb)  # Velocidade após a transição da desaceleração
    ...
    _, _, _, _, t_values, acceleration, velocity, position = simulate_car(F_pedal, pos_acel_max, pos_freio, pos_desejada)
    ...
    for i, p in enumerate(position):
        if i % 5 == 0:  # Salvar a cada 5 pontos (ajustado conforme solicitado)
