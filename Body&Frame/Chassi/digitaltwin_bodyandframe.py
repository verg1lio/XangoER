...
# ---------------------
    ...
    def tubo_oco2(self, p1, p2, tube_type, retornar_partes=False):
            """
            Create a hollow tube using cadquerry commands with intersection fix.
            ...
            # --- AQUI MUDA ---
            # Posicionamento (precisamos posicionar as duas partes)
            start_point_adjusted = p1_mm - (vec_normalized * overlap)
            
            # Função auxiliar para mover
            def mover(obj):
                z_axis = np.array([0, 0, 1])
                axis = np.cross(z_axis, vec_normalized)
                axis_length = np.linalg.norm(axis)
                if axis_length < 1e-6:
                    if np.dot(z_axis, vec_normalized) > 0: return obj.translate(tuple(start_point_adjusted))
                    else: return obj.rotate((0,0,0), (1,0,0), 180).translate(tuple(start_point_adjusted))
                else:
                    angle = np.degrees(np.arccos(np.dot(z_axis, vec_normalized)))
                    return obj.rotate((0, 0, 0), tuple(axis / axis_length), angle).translate(tuple(start_point_adjusted))

            part_outer = mover(outer_solid)
            part_inner = mover(inner_solid)

            if retornar_partes:
                # Retorna tupla (Solidão, Ar_interno)
                return (part_outer, part_inner)
            
            # Comportamento antigo (retorna o tubo já cortado)
            tubo_final = part_outer.cut(part_inner)
            
            if not tubo_final.val().isValid(): return None
            return tubo_final

    def create_step_complete2(self, nome="chassi"):
            import cadquery as cq
            from cadquery import exporters
            ...
            print(f"\n--- GERAÇÃO VIA MÉTODO 'SUBTRAÇÃO ITERATIVA' ({len(self.elements)} elementos) ---")

            # 1. Gerar geometrias puras
            for i, element in enumerate(self.elements):
                node_a, node_b, tube_type = element
                p1 = self.nodes[node_a]
                p2 = self.nodes[node_b]

                # Pede as partes separadas
                resultado = self.tubo_oco2(p1, p2, tube_type, retornar_partes=True)

                if resultado:
                    out_s, in_s = resultado
                    if out_s.val().isValid() and in_s.val().isValid():
                        lista_externos.append(out_s)
                        lista_internos.append(in_s)

            if not lista_externos:
                print("❌ Erro: Nenhum tubo gerado.")
                return

            # 2. Unir a "Casca" Externa (Maciça)
            print(f"Unindo {len(lista_externos)} tubos para criar o Sólido Mestre (Maciço)...")
            
            sólido_mestre = None
            try:
                # Tenta unir tudo de uma vez (mais rápido)
                wp_ext = cq.Workplane("XY")
                for o in lista_externos: wp_ext = wp_ext.add(o)
                sólido_mestre = wp_ext.combine() # Sem tolerância para garantir precisão
                
                if not sólido_mestre.val().isValid():
                    raise Exception("União externa resultou em sólido inválido.")
                print("✅ Sólido Mestre criado com sucesso!")

            except Exception as e:
                print(f"⚠️ Falha na união em massa: {e}. Tentando união iterativa (mais lenta)...")
                # Fallback: Une um por um se o combine falhar
                sólido_mestre = lista_externos[0]
                for i, tubo in enumerate(lista_externos[1:]):
                    try:
                        sólido_mestre = sólido_mestre.union(tubo)
                    except:
                        print(f"  ❌ Falha ao unir tubo externo {i+1}. Pulando.")
            
            # 3. Furar o Sólido Mestre (Um por um)
            print(f"Perfurando o Sólido Mestre com {len(lista_internos)} núcleos de ar...")
            
            tubos_furados = 0
            tubos_falhos = 0

            # Loop de "Broca": subtrai cada ar interno do sólido principal
            for i, nucleo_ar in enumerate(lista_internos):
                try:
                    # O segredo: Cut iterativo
                    novo_solido = sólido_mestre.cut(nucleo_ar)
                    
                    if novo_solido.val().isValid():
                        sólido_mestre = novo_solido
                        tubos_furados += 1
                    else:
                        # Se o resultado for inválido, ignoramos este corte e mantemos o sólido anterior
                        tubos_falhos += 1
                        print(f"  ⚠️ Aviso: Corte inválido no tubo {i+1}. Este tubo ficará maciço.")
                except Exception as ex:
                    tubos_falhos += 1
                    print(f"  ⚠️ Erro ao cortar tubo {i+1}: {ex}")

            print(f"\n--- RELATÓRIO FINAL ---")
            print(f"✅ Tubos ocos: {tubos_furados}")
            print(f"⚠️ Tubos maciços (falha no corte): {tubos_falhos}")

            # 4. Exportação
            try:
                # Opcional: Limpeza final de topologia
                # sólido_mestre = sólido_mestre.clean() 
                
                exporters.export(sólido_mestre, f"{nome}.step")
                print(f"\n✅ ARQUIVO GERADO: {nome}.step (Sólido Único)")
            except Exception as ex:
                print(f"\n❌ ERRO FATAL AO EXPORTAR: {ex}")
                print("Salvando como Compound de emergência...")
                comp = cq.Compound.makeCompound(lista_externos)
                exporters.export(comp, f"{nome}_emergencia.step")
