    ...
    def combined_edos(self, t: float, x: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if (self.vehicle is not None) and (self.transmission is not None) and (self.tire is not None):
                try:
                    # 1. Calcular Torque e Forças Resistivas
                    ...
                    # 2. Calcular Força de Tração Ideal (vinda do motor)
                    ...
                    # --- RESOLUÇÃO DO LOOP ALGÉBRICO (Iteração) ---
                    ...
                    traction_force_lim_est = np.sign(traction_force_ideal) * min(abs(traction_force_ideal), abs(Fx_max_est))
                    force_result_est = traction_force_lim_est - resistance_force
                    ...
                    # Lógica de parada (freio/inercia zero)
    ...
    def simulate(self, t0: float = 0.0, tf: Optional[float] = None) -> Dict[str, Any]:
        ...
        for idx, t in enumerate(sol.t):
            ...
            if self.vehicle and self.transmission and self.tire:
                ...
                self.fz_hist.append(Fz)
                self.longitudinal_force_hist.append(Fx_max) # Força máxima do pneu
