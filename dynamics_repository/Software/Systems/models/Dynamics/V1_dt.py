...
class Drivetrain:
    ...
    Parameters
    ---------
    gravitational_center_x: centro de mass no eixo x
    gravitational_center_y: centro de mass no eixo y
    mass: mass do veículo
    axis_distance: distância entre eixos
    friction_coef: coeficiente de atrito
    ...
    def __init__(self, gravitational_center_x, gravitational_center_y, mass, axis_distance, friction_coef, tire_radius, ideal_acceleration, primary_reduction, reducao_unica, rpm, torque, cp):
        self.gravitational_center_x = gravitational_center_x
        self.gravitational_center_y = gravitational_center_y
        self. mass = mass
        self.axis_distance =axis_distance
        self.friction_coef = friction_coef
        self.tire_radius = tire_radius
        self.ideal_acceleration = ideal_acceleration
        self.primary_reduction = primary_reduction
    ...
    def CalculateOutputs(self):
        ...
        - Aceleração primária real:         Aceleração real do veículo com base na força trativa e na mass.
        ...
        - Força trativa ideal:             Força trativa ideal baseada na aceleração ideal e na mass.
        ...
        '''
        peso = self.mass * 9.81
        reacao_traseira = (peso * (self.gravitational_center_x * 0.001)) / (self.axis_distance * 0.001)
        reacao_dianteira = self.mass * 9.81 - reacao_traseira
        forca_trativa = (reacao_traseira * self.friction_coef) / (1 - ((self.gravitational_center_y * 0.001) * self.friction_coef) / (self.axis_distance * 0.001))
        transferencia_longitudinal = (forca_trativa * self.gravitational_center_y * 0.001) / (self.axis_distance * 0.001)
        carga_traseira = reacao_traseira + transferencia_longitudinal
        pico_torque_traseiro = carga_traseira * self.tire_radius * 0.001 * self.friction_coef
        carga_pneu = carga_traseira / 2
        torque_pneu = forca_trativa * self.tire_radius * 0.001
        reducao_final = self.primary_reduction * self.reducao_unica
        torque_necessario_motor = pico_torque_traseiro / (self.reducao_unica * self.primary_reduction * self.cp)
        aceleracao_primaria_real = (forca_trativa / self.mass) / 9.81
        aceleracao_primaria_ideal = self.ideal_acceleration * 9.81
        aceleraco_real_final = aceleracao_primaria_real * 9.81
        forca_trativa_ideal = self.mass * aceleracao_primaria_ideal
        torque_pneu_ideal = forca_trativa_ideal * self.tire_radius * 0.001
        torque_motor_ideal = torque_pneu_ideal / (self.reducao_unica * self.primary_reduction * self.cp)
        transferencia_carga_ideal = (forca_trativa_ideal * self.gravitational_center_y) / self.axis_distance
        transferencia_carga_real = (forca_trativa * self.gravitational_center_y) / self.axis_distance
    ...
    def showResults(self):
        ...
        - Aceleração primária real (g):     Aceleração real do veículo com base na força trativa e na mass, expressa como múltiplo da aceleração da gravidade (g).
        ...
        - Força trativa ideal:             Força trativa ideal baseada na aceleração ideal e na mass em Newtons (N).
        ...
        Example:
        ------
        dt_model = Drivetrain(gravitational_center_x = 853, gravitational_center_y = 294,  mass = 347, axis_distance = 1567, friction_coef = 0.9, tire_radius = 259, ideal_acceleration = 1.2, primary_reduction = 2.12, reducao_unica = 2.76, rpm = 2625, torque= 244.14, cp = 2.22)
    ...
    def CarPerformance(self):
        ...
        if self.new_rpm:
            variacao = range(int(self.rpm), int(self.new_rpm), 30)
        ...
        for rpm in variacao:
            ...
            forca_trativa = ((self.torque * self.primary_reduction * self.reducao_unica * self.cp) / (self.tire_radius * 0.001)) * rendimento_transmissao
        
            # Cálculo da velocidade angular (rad/s)
            velocidade_angular = (rpm * 2 * np.pi) / (60 * self.primary_reduction * self.reducao_unica * self.cp)
            
            # Cálculo da velocidade linear (km/h)
            velocidade_linear = ((velocidade_angular * (self.tire_radius * 0.001)) * transmissao_motor_roda) * 3.6
    ...
    def infoCarPerformance(self):
        ...
        text = "Força Trativa [N]\tVelocidade Angular [rad/s]\tVelocidade Linear [km/h]"
        
        for param in performance:
            text+=f"\n{param['forca_trativa']}\t{param['va']}\t{param['velocidade_linear']}"
        
        text += "\nForça de Arrasto [N]\tResistência de Rolamento [N]\tForça Final [N]"
        
        for param in performance:
            text+=f"\n{param['fa']}\t{param['rr']}\t{param['forca_final']}"
    ...
    def HalfShaftsSizing(self, fsi=1.25, tet=786, tec=471.6, dif=1):
        ...
        torque_max_semieixo = torque_max_motor * self.primary_reduction * self.reducao_unica * self.cp * dif
        ...
        return torque_max_motor, torque_max_semieixo, torque_max_projeto, diametro_semieixo, fsi, fator_seguranca_obtido, fs1p

    def resultados_HalfShaftsSizing(self):
        torque_max_motor, torque_max_semieixo, torque_max_projeto, diametro_semieixo, fsi, fator_seguranca_obtido, fs1p = self.HalfShaftsSizing() 
        # Print dos resultados obtidos
        text = f"""
        Dimensionamento dos Semieixos:
        Torque máximo do motor: {torque_max_motor} N
        Torque máximo nos semieixos: {torque_max_semieixo} Nm
        Torque máximo de projeto:", {torque_max_projeto} Nm
        Diâmetro dos semieixos:", {diametro_semieixo} mm
        Fator de segurança ideal: {fsi}
        Fator de segurança obtido: {fator_seguranca_obtido}
        Fator de segurança para 1 polegada: {fs1p}
        """
    ...
    def uniteExample():
        dt_model = Drivetrain(
            ...
            347,  # kg
            1567,  # mm
            0.9 , # coeficiente de atrito
            259,  # mm
            1.2,  # g
            2.12,  # redução primária
            2.76,
            8,
            244.14,
            2.22)
        ...
        slip_ratio = Tire.slip_ratio_1(velocidade_angular, 0.259, 30)
