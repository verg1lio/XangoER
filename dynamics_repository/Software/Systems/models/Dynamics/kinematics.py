...
class Kinematics:
    ...
    def plotar_cinematica_base64(self):
        """
        Gera os frames da cinemática do mecanismo em formato base64.

        Returns
        -------
        frames_base64 : list of str
            Lista de imagens (em base64) de cada passo da cinemática.
        """
        ...
        for i in range(len(self.Py)):
            ...
            # Barras
            ax.plot([self.Ox, self.Ax[i]], [self.Oy, self.Ay[i]], 'b', linewidth=2, label='Barra de Entrada')
            ax.plot([self.Ax[i], self.Bx[i]], [self.Ay[i], self.By[i]], 'b', linewidth=2, linestyle='dotted', label='Barra de Acoplamento')
            ax.plot([self.Bx[i], self.Cx], [self.By[i], self.Cy], 'b', linewidth=2, label='Barra de Saída')
            ax.plot([self.Ax[i], self.Px[i]], [self.Ay[i], self.Py[i]], 'r', linewidth=1, label='Barra Adicional')
            ax.plot([self.Bx[i], self.Px[i]], [self.By[i], self.Py[i]], 'g', linewidth=1, label='Barra Adicional')

            # Textos
            ax.text(self.Ox, self.Oy, f'Âncoragem ({self.Ox:.2f}, {self.Oy:.2f})', fontsize=8, ha='right')
            ax.text(self.Ax[i], self.Ay[i], f'A ({self.Ax[i]:.2f}, {self.Ay[i]:.2f})', fontsize=8, ha='left')
            ax.text(self.Bx[i], self.By[i], f'B ({self.Bx[i]:.2f}, {self.By[i]:.2f})', fontsize=8, ha='left')
            ax.text(self.Cx, self.Cy, f'C ({self.Cx:.2f}, {self.Cy:.2f})', fontsize=8, ha='right')
            ax.text(self.Px[i], self.Py[i], f'Cubo ({self.Px[i]:.2f}, {self.Py[i]:.2f})', fontsize=8, ha='left')
            ax.text((self.Ax[i] + self.Bx[i]) / 2, (self.Ay[i] + self.By[i]) / 2, f'{angulo_camber:.2f}°', fontsize=12, ha='center')
        ...
        for i in range(len(self.Py) - 1, -1, -1):
            ...
            fig, ax = plt.subplots(figsize=(6, 6))
            ax.plot([self.Ox, self.Ax[i]], [self.Oy, self.Ay[i]], 'b', linewidth=2)
            ax.plot([self.Ax[i], self.Bx[i]], [self.Ay[i], self.By[i]], 'b', linewidth=2, linestyle='dotted')
            ax.plot([self.Bx[i], self.Cx], [self.By[i], self.Cy], 'b', linewidth=2)
            ax.plot([self.Ax[i], self.Px[i]], [self.Ay[i], self.Py[i]], 'r', linewidth=1)
            ax.plot([self.Bx[i], self.Px[i]], [self.By[i], self.Py[i]], 'g', linewidth=1)
        ...
        # Calcular informações de câmber
        camber_inicial = self.calcular_camber(self.Ax[0], self.Ay[0], self.Bx[0], self.By[0])
        camber_estatico = self.calcular_camber(self.Ax[int(self.num_points/2)], self.Ay[int(self.num_points/2)],
                                            self.Bx[int(self.num_points/2)], self.By[int(self.num_points/2)])
        camber_final = self.calcular_camber(self.Ax[-1], self.Ay[-1], self.Bx[-1], self.By[-1])
        taxa_variacao = (camber_final - camber_inicial) / (self.Py_step * len(self.Py))

        text = f"""
        Ângulo de câmber rebound: {camber_inicial:.2f}°
        Ângulo de câmber estático: {camber_estatico:.2f}°
        Ângulo de câmber bump: {camber_final:.2f}°
        Taxa de variação de câmber: {taxa_variacao:.2f}°/mm
        """
        

        return frames_base64, text
    ...
    def plot_damper(self, damper_Vx_values, damper_Vy_values):
        ...
        ax.plot(damper_Vx_values, damper_Fx_values, label='Velocidade')
        ax.set_title('Força em função da velocidade em X')
        ax.set_xlabel('Velocidade [mm/s]')
        ax.set_ylabel('Força [N]')
        ax.grid(True)
        ax.legend()

        ax.plot(damper_Vy_values, damper_Fy_values, label='Velocidade')
        ax.set_title('Força em função da velocidade em Y')
        ax.set_xlabel('Velocidade [mm/s]')
        ax.set_ylabel('Força [N]')
        ax.grid(True)
        ax.legend()
    ...
    def plot_spring(self, spring_x_values, spring_y_values):
        ...
        fig, ax = plt.subplots(figsize=(8, 6))
        ax.plot(spring_x_values, spring_Fx_values, label='Deformação')
        ax.set_title('Força em função da deformação em X')
        ax.set_xlabel('Deformação [mm]')
        ax.set_ylabel('Força [N]')
        ax.grid(True)
        ax.legend()

        ax.plot(spring_y_values, spring_Fy_values, label='Deformação')
        ax.set_title('Força em função da deformação em Y')
        ax.set_xlabel('Deformação [mm]')
        ax.set_ylabel('Força [N]')
        ax.grid(True)
        ax.legend()
