...
class SimulacaoBateria:
    ...
    def q_dot(self, t):
        ...
        if t < 5:
            # Largada com aceleração forte
            return base + (pico_aceleracao - base) * np.exp(-((t - 2.5) ** 2) / 1.5)
        
        elif 5 <= t < 15:
            # Velocidade constante (reta)
            return medio
        
        elif 15 <= t < 20:
            # Primeira frenagem
            return recuperacao + 500 * np.cos(2 * np.pi * (t - 15) / 5)
        
        elif 20 <= t < 27:
            # Nova aceleração (subida suave de velocidade)
            return base + 4000 * np.sin((t - 20) * np.pi / 7)
        
        elif 27 <= t < 35:
            # Estável de novo
            return medio + 1000 * np.sin(2 * np.pi * (t - 27) / 8)
        
        elif 35 <= t < 40:
            # Nova frenagem
            return recuperacao + 700 * np.cos(2 * np.pi * (t - 35) / 5)

        elif 40 <= t < 50:
            # Trecho com aceleração forte (vários aumentos consecutivos)
            return base + (pico_aceleracao - base) * np.exp(-((t - 45) ** 2) / 6)

        elif 50 <= t < 55:
            # Queda rápida de velocidade (desaceleração)
            return recuperacao
        
        elif 55 <= t < 58:
            # Motor quase parado
            return parado + 200 * np.sin((t - 55) * np.pi)
        
        elif 58 <= t < 60:
            # Pequena retomada até fim do ciclo
            return medio
