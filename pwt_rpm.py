...
class Motor:
    ...
    def source_voltage(self,): #fabricio
    ...
    def direct_voltage_and_quadrature(self, vs1, vs2, vs3): #iago
    ...
    def calculate_derivatives(self, vsd, vsq, vso): #gabriel
    ...
    def update_fluxes_and_currents(self, dervfsd, dervfsq, dervfrd, dervfrq, deriso): #igor
    ...
    def calculate_electromagnetic_torque(self,): #iago
    ...
    def currents_and_fluxes_phases(self, fso): #igor
    ...
    def mechanical_speed(self,): 
        ...
        self.wm = self.wm + (self.ce - self.cl - self.wm * self.kf) * self.h / self.jm
        return self.wm
    ...
    def rpmf(self):
        rpm = 30 *(self.wm / self.tmax) * self.t / np.pi
        return rpm 

    def outputs(self, is1, is2, is3, fs1, fs2, fs3, fso, cm, vso, vsd, vsq, rpm):
    ...
    def simulate(self):
        ...
        while self.t < self.tmax:
            ...
            if self.t >= self.tp:
                self.tp += self.hp
                self.outputs(is1, is2, is3, fs1, fs2, fs3, fso, cm, vso, vsd, vsq, rpm)
    ...
    def plot_motor(self):
        ...
        plt.figure(5)
        plt.plot(self.tempo, self.rota, label='RPM')
        plt.title('RPM')
        plt.legend()
        plt.xlabel('Time (s)')
        plt.ylabel('RPM')
    ...
    def transfer_function(self): #fabricio
        """
        Compute the transfer function for the motor.

        The transfer function is given by H(s) = (msr * p) / (Js^2 + 2 * kf * s + Rs + Ls),
        where msr is the mutual inductance between stator and rotor, p is the number of pole pairs,
        Js is the moment of inertia, kf is the friction coefficient, Rs is the stator resistance and
    ...
    def state_space_representation(self): #gabriel
    ...
    def print_state_space(self): #gabriel
