...
float fsb, fra, frb, frd, frq, fsd, fsq, flrr = 0.8, flsr = 0.88, rlmd;
...
float kptot = 0., xerf = 0.0, xerw = 0.0, erf_var = 0.0, erw = 0.0;
/*Alteração no código diferente do SALVADORI, erf é uma função do math.c, para contornar, escrevi "erf" como "erf_var"*/
...
float aux[3], gtf[3], xf[7], xd[5][7], dv[5][7], xa[5][7];
float
    esd1 = 0.0,
    esq1 = 0.0, esq2 = 0.0, esq3 = 0.0, ts = 0.0, tr = 0.0;
...
bloc1b()
{
    ...
    if (cont >= te)
    {
        ...
        if (regul == 1)
        {
            ...
            dei[3] = frqi - frq;
            /* */
            isdi = erf_var + (fkp + fki) * dei[2];
            erf_var = erf_var + fki * dei[2];
            if (erf_var >= 15.)
                erf_var = 15;
            if (erf_var <= -15.)
                erf_var = -15.;
        ...
        /* */
           ...
           // AQUI ENCERRA O SWITCH CONTROLES
...
bloco2()
{
    ...
    if (psi >= pid)
        psi = psi - pid;
    /* */
    vsd = rq23 * (vsa * cos(psi) + vsb * cos(psi - pid / 3.) + vsc * cos(psi + pid / 3.));
    ...
    for (j = 0; j <= 3; ++j)
    {
        ...
        dv[j][2] = vrd - (cd * rr) * (xd[j][2] * ls - xd[j][0] * lm) + (wd - xd[j][5]) * xd[j][3];
        dv[j][3] = vrq - (cd * rr) * (xd[j][3] * ls - xd[j][1] * lm) - (wd - xd[j][5]) * xd[j][2];
        dv[j][4] = xd[j][5];
    ...
    fsbf = (fsqf * cos(psi) + fsdf * sin(psi));
