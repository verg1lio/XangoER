...
# ==============================================================================
initial_speed = 20.0
# Dummy data para evitar erro no método original plot_graph_slip_ratio caso chamado
pedal_forces = np.linspace(0, 100, 100) 

# ==============================================================================
# CÓDIGO 1: MODELO DE PNEU (PACEJKA) E MECANISMO DE DIREÇÃO
# ==============================================================================
...
# ==============================================================================
# CÓDIGO 2: CINEMÁTICA 3D
# ==============================================================================

class Kinematics:
    # CORREÇÃO: Adicionados valores padrão para evitar NoneType error
    def __init__(self, L0=500, L1=500, L2=450, L3=500, L_pushrod=700, theta_pushrod=np.radians(340), alpha=np.radians(45), theta2_start=75, theta2_end=105, theta2_step=0.6, space=200, mirror=230):
        self.L0 = L0
        self.L1 = L1 
        self.L2 = L2
        self.L3 = L3
        self.L_pushrod = L_pushrod
        self.theta_pushrod = theta_pushrod
        self.alpha = alpha
        self.L_AP = L2 / np.sqrt(2)
        # Ajuste no passo para reduzir carga no notebook
        self.theta2 = np.arange(theta2_start, theta2_end, 2.0)
        self.theta2_rad = np.radians(self.theta2)
        ...
        ########################### pushrod
        self.Ax_push, self.Az_push = [], []
        self.Bx_push, self.Bz_push = [], []
    ...
    def calcular_cinematica(self):
        for i in range(len(self.theta2)):
            ...
            ############################### Pushrod
            Ax_push_i = self.L1 * np.sin(self.theta2_rad[i])
            Az_push_i = self.L1 * np.cos(self.theta2_rad[i])
            Bx_push_i = Ax_push_i + self.L_pushrod * np.sin(self.theta_pushrod)
            Bz_push_i = Az_push_i + self.L_pushrod * np.cos(self.theta_pushrod)
            ...
            ################################ Pushrod
            self.Ax_push.append(Ax_push_i)
            self.Az_push.append(Az_push_i)
            self.Bx_push.append(Bx_push_i)
            self.Bz_push.append(Bz_push_i)
    ...
    def plotar_cinematica(self):
        ...
        ###################### Pushrod
        self.Ay_push = [0] * len(self.theta2)
        self.By_push = [0] * len(self.theta2)
        ...
        for i in range(len(self.theta2)):
                ...
                ################ Barra preta
                ax.plot([self.Ax_push[i], (self.Bx_push[i] )], [self.Ay_push[i], self.By_push[i]  ], [self.Az_push[i], self.Bz_push[i]], 'black', linewidth=2)  # Acoplamento
                #### ESPELHAMENTO
                ...
                ################ Barra preta espelhada
                ax.plot([(self.Ax_push[i] * -1 ) - self.mirror, ((self.Bx_push[i] )* -1) - self.mirror], [self.Ay_push[i], self.By_push[i]], [self.Az_push[i], self.Bz_push[i]], 'black', linewidth=2) # Acoplamento espelhado
        ...
        for i in range(len(self.theta2)-1, -1, -1):
            ...
            ###################pushrod
            ax2.plot([self.Ax_push[i], self.Bx_push[i]], [self.Ay_push[i], self.By_push[i]], [self.Az_push[i], self.Bz_push[i]], 'black', linewidth=2)  # Acoplamento
            #### ESPELHAMENTO
            ...
            ###################pushrod
            ax2.plot([(self.Ax_push[i] * -1) - self.mirror, (self.Bx_push[i]* -1) - self.mirror], [self.Ay_push[i], self.By_push[i]], [self.Az_push[i], self.Bz_push[i]], 'black', linewidth=2)
...
# --- Cálculo das métricas com índices dinâmicos ---
        n_frames = len(self.Ax)
        idx_inicial = 0
        idx_estatico = n_frames // 2  # Pega o índice do meio da lista
        idx_final = n_frames - 1      # Pega o último índice disponível

        # Imprimindo os ângulos de câmber
        camber_inicial = self.calcular_camber(self.Ax[idx_inicial], self.Az[idx_inicial], self.Bx[idx_inicial], self.Bz[idx_inicial])
        camber_estatico = self.calcular_camber(self.Ax[idx_estatico], self.Az[idx_estatico], self.Bx[idx_estatico], self.Bz[idx_estatico])
        camber_final = self.calcular_camber(self.Ax[idx_final], self.Az[idx_final], self.Bx[idx_final], self.Bz[idx_final])
        
        taxa_variacao = (camber_final - camber_inicial) / n_frames

        print(f"\n--- Resultados da Suspensão ---")
        print(f"Frames calculados: {n_frames}")
        print(f"Ângulo de câmber bump (inicial): {camber_inicial:.2f}°")
        print(f"Ângulo de câmber estático (meio): {camber_estatico:.2f}°")
        print(f"Ângulo de câmber rebound (final): {camber_final:.2f}°")
        print(f"Taxa de variação de câmber: {taxa_variacao:.4f}°/passo")


# ==============================================================================
# CÓDIGO 3: OTIMIZAÇÃO DE GEOMETRIA (ORIGINAL)
# ==============================================================================
...
def executar_codigo_3_otimizacao():
    print("--- EXECUÇÃO: CÓDIGO 3 (Otimização) ---")
    print("Iniciando a otimização...")

    # Lista de 30 tuplas (min, max) para os 10 pontos.
    # !!! ADAPTAÇÃO: Correção de bounds onde min == max para evitar crash do scipy !!!
    raw_bounds = [
        # UBJ (x,y,z)
        (0, 0), (0, 0), (320, 320),
        # LBJ (x,y,z)
    ...
    # Processamento dos bounds para garantir min < max
    bounds = []
    for b in raw_bounds:
        if b[0] == b[1]:
            bounds.append((b[0], b[1] + 1e-6))
        else:
            bounds.append(b)
...
# ==============================================================================
# EXECUTANDO TUDO NO JUPYTER (Copie esta célula)
# ==============================================================================

# 1. Gráficos Estáticos do Code 1
print("\n--- GRÁFICOS ESTÁTICOS: PNEU ---")
# Dados experimentais
...
# 3. Animação Code 2 (Suspensão 3D)
print("\n--- SUSPENSÃO 3D ---")
kinematics = Kinematics(L0=500, L1=500, L2=450, L3=500, L_pushrod = 700)
kinematics.calcular_cinematica()
kinematics.plotar_cinematica()

# 4. Otimização Code 3 (Texto)
executar_codigo_3_otimizacao()
