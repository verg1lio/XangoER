...
class Estrutura:
    ...
    def obter_propriedades(self, tube_nome):
        """Get tube properties from cache"""
        if tube_nome not in self.tube_properties_cache:
            raise ValueError(f"Tubo '{tube_nome}' não encontrado.")
            
        props = self.tube_properties_cache[tube_nome]
        E = props['E']
        G = props['G']
    ...
    def element(self, element):
        ...
        # Para convertir para timoshenko basta trocar c3 por c4,onde tem (4 * L_e**2 * c3) substitui por (t1* L_e**2 * c4) e onde tiver (2 * L_e**2 * c3) por (t2* L_e**2 * c4))
        k_e= np.array([
    ...
    def compute_Kf_Kt(self, lfNode, lrNode, rfNode, rrNode, lcNode, rcNode):
            ...
            F_global[2+lcNode*6] = -F/2                                                             #Força distribuída nos nós centrais do chassi (nodes 22 and 23)
...
print("\\n Frequências Naturais (ω) da estrutura:")#print(frequencias)
