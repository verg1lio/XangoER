...
class BrakeSystem:
    ...
    def calculate_params(self, pedal_force):
        ...
        Awc = (self.pi * ((self.Dwc / 2) ** 2))  # Área do cilindro de roda
        Acm = (self.pi * ((self.Dcm / 2) ** 2))  # Área do cilindro mestre
    ...
    def apply_brake(self, pedal_force, initial_speed):
        ...
        # Calcular a pressão do cilindro mestre
        pressao_cilindro_mestre = pedal_force / Acm
        # Calcular a pressurização do fluido
        pressao_fluido = pressao_cilindro_mestre
        # Calcular a transmissão de pressão da linha de freio
        transmissao_pressao = pressao_fluido * Awc
        # Calcular a força de aperto da pinça
        forca_aperto_pinca = transmissao_pressao
        # Calcular a força de atrito da pastilha de freio
        forca_atrito_pastilha = forca_aperto_pinca * self.atrito_coeficiente
        # Calcular o torque do disco de freio
        torque_disco_freio = forca_atrito_pastilha * self.red
        # Calcular a força de frenagem considerando todos os fatores
        ...
        # Calcular a velocidade angular do pneu a partir do torque de freio
        ...
        angular_velocity = initial_speed / self.Rdp  # Velocidade angular
        max_deceleration = initial_speed / 2  # Ajuste este valor conforme necessário
        new_angular_velocity = angular_velocity - min(angular_deceleration, max_deceleration)  # Nova velocidade angular
...
params = {
    ...
    'atrito_coeficiente': 0.25,  # Coeficiente de atrito da pastilha <--- Reduzido para menor força de frenagem
    'red': 0.60,  # Raio efetivo do disco [m] <--- Reduzido para menor torque de frenagem
    'Mt': 250,  # Massa total do veículo [kg]
    'm_wheel': 15,  # Massa da roda [kg] <-- estimativa
    'L': 1.5  # Distância entre eixos [m] <--- Adicionado o parâmetro 'L'
}
...
class Dynamics:
    def _init_(self, spring_type, spring_k, spring_F, spring_non_lin_coef, tire_Fz, tire_Sa, tire_Ls):
        # Modelo de mola
...
tire_Ls = new_angular_velocity / (initial_speed / params['Rdp'])  # Slip ratio baseado na nova velocidade angular
