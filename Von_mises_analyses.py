...
class Estrutura:
    ...
    def compute_strain2(self, displacements):
        ...
        for element in self.elements:
            ...
            D_T = np.array([
                D[6 * node1], D[6 * node1 + 1], D[6 * node1 + 2],
                D[6 * node1 + 3], D[6 * node1 + 4], D[6 * node1 + 5],
                D[6 * node2], D[6 * node2 + 1], D[6 * node2 + 2],
    ...
    def plot_colored_wireframe(nodes, elements, scalar_values, graphtitle='Wireframe plot', scalelabel='Your variable here', colormap='jet'):
        ...
        norm = plt.Normalize(vmin=np.min(scalar_values), vmax=np.max(scalar_values)/2)
...
j = 0.04
k = 0.03
nodes = np.array ([[64*i, 0*j, 0*k] , [64*i, 16*j, 0*k] ,[64*i, 0*j, 16*k] , [64*i, 16*j, 16*k] ,[59*i, 0*j, 7*k] , [59*i, 16*j, 7*k] , [64*i, 0*j, 3*k] , [64*i, 16*j, 3*k] , [50*i, 0*j, 1*k] , [50*i, 16*j, 1*k] , [38*i, 2*j, 1*k] , [38*i, 14*j, 1*k] , [38*i, 0*j, 3*k] , [38*i, 16*j, 3*k] , [41*i, 0*j, 12*k] , [41*i, 16*j, 12*k] , [38*i, 1*j, 24*k] , [38*i, 15*j, 24*k] , [21*i, 0*j, 18*k] , [21*i, 16*j, 18*k] , [23*i, 0*j, 8*k] , [23*i, 16*j, 8*k] , [23*i, 0*j, 0*k] , [23*i, 16*j, 0*k] , [15*i, 0*j, 7*k] , [15*i, 16*j, 7*k] , [8*i, 0*j, 3*k] , [8*i, 16*j, 3*k] , [0*i, 4*j, 7*k] , [0*i, 12*j, 7*k] , [0*i, 4*j, 3*k] , [0*i, 12*j, 3*k] , [0*i, 4*j, 14*k],[0*i, 12*j, 14*k] , [11*i, 1*j, 22*k] , [11*i, 15*j, 22*k] , [19*i, 1*j, 40*k] , [19*i, 15*j, 40*k] , [18*i, 8*j, 45*k] , [38*i, 8*j, 26*k]])  
...
#nodes = np.array ([    [0,     0,      0],    [0,     0.375,    0],    [0,     0.700,    0],    [1.500,  0.375,    0],    [1.500,  0,      0],    [1.500,  0.700,    0]])
...
ordem_displacements = np.array(displacements).shape
print(f'a ordem da matriz dos deslocamentos é {ordem_displacements}')
...
#print(von_mises)
Estrutura.plot_colored_wireframe(nodes,elements,von_mises,'Von Mises Stress', 'Stress[Pa]')
